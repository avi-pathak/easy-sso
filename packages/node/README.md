...
Google ships as a second built-in provider via `googleAuth` (or `GoogleProvider`
+ `createAuthMiddleware`). It validates Google-issued OIDC **ID tokens** against
Google's live JWKS (`https://www.googleapis.com/oauth2/v3/certs`), issuer
`https://accounts.google.com`, and `aud = <your OAuth client id>`.

```ts
import { googleAuth, requireAuth } from "@easy-sso/node";

app.use(
  googleAuth({
    clientId: process.env.GOOGLE_CLIENT_ID!, // expected `aud`
    audience: ["web-client-id", "ios-client-id"], // optional: several client IDs
    hostedDomains: ["yourcompany.com"], // optional: restrict to a Workspace domain (hd)
  }),
);
app.get("/me", requireAuth(), (req, res) => res.json(req.user));
```
...
`AuthUser.tenantId`. `GoogleAuthConfig`: `{ clientId, audience?, hostedDomains?,
clockToleranceSec?, claims?, jwks?, fetch?, clock? }`.
...
| `googleAuth` | `(config, options?) => RequestHandler` | Google provider + middleware in one. |
